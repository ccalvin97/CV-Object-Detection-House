...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    ##  opt.global_rank = -1, opt.world_size = 1
    ...
    with open(opt.data) as f:
        data_dict = yaml.load(f, Loader=yaml.SafeLoader)  # data dict, Loader=yaml.SafeLoader -> Load YAML file
...
if __name__ == '__main__':
    ...
    # Set DDP variables
    opt.world_size = int(os.environ['WORLD_SIZE']) if 'WORLD_SIZE' in os.environ else 1  # 單機單卡 沒有WORLD_SIZE
    opt.global_rank = int(os.environ['RANK']) if 'RANK' in os.environ else -1            # 單機單卡 沒有RANK
    ...
    with open(opt.hyp) as f:
        hyp = yaml.load(f, Loader=yaml.SafeLoader)  # load hyps, Loader=yaml.SafeLoader ——》 Load YAML File
